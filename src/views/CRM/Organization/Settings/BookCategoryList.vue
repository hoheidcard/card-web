      ...
      <div class="table-header">
        <div class="left">
          <div class="title-row">
            ...
            <div class="titles">
              <div class="sub-title">Books Category : List</div>
              <h5 class="main-title">Our Books Category</h5>
            </div>
          </div>
        </div>
        <div class="right">
          ...
          <div class="icons">
            <div class="icon tooltip-wrapper" @click.prevent="openAddModal">
              <icon-plus></icon-plus>
              <div class="tooltip">Add Books Category</div>
            </div>
          </div>
        </div>
      </div>
      <div class="table-body mt-16">
        <div class="table-outer">
          <table class="table">
            ...
            <tbody>
              <tr v-for="(item, index) in storeVar.bookCategory" :key="index">
                <td>{{ index + 1 }}</td>
                <td>{{ item.name }}</td>
        ...
        <div class="table-no-data" v-if="storeVar.bookCategory.length <= 0">
    ...
    >
      ...
      <div class="modal-footer">
        <div class="btns">
          ...
          <div class="btn white-btn" @click="addBookCategory">Submit</div>
        </div>
      </div>
    </Modal>
...
function openEditModal(item) {
  ...
  formVar.bookCategoryId = item.id;
  formVar.name = item.name;
}
function openDeleteModal(id) {
  formVar.bookCategoryId = id;
...
function addBookCategory() {
  ...
  if (!formVar.editMode) {
    store.dispatch("OrganizationSettings/addBookCategory", {
      name: formVar.name,
      settingId: formVar.organizationSettingId,
    });
  } else {
    store.dispatch("OrganizationSettings/updateBookCategory", {
      id: formVar.bookCategoryId,
      name: formVar.name,
      settingId: formVar.organizationSettingId,
    });
  }
...
function submitDelete() {
  store.dispatch("OrganizationSettings/deleteBookCategory", {
    id: formVar.bookCategoryId,
    status: "DELETED",
  });
}
