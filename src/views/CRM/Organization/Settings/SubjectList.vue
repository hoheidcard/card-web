      ...
      <div class="table-body mt-16">
        <div class="table-outer">
          <table class="table">
            ...
            <tbody>
              <tr v-for="(item, index) in storeVar.subject" :key="index">
                <td>{{ index + 1 }}</td>
                <td>{{ item.name }}</td>
        ...
        <div class="table-no-data" v-if="storeVar.subject?.length <= 0">
...
function addSubject() {
  ...
  if (!formVar.editMode) {
    store.dispatch("OrganizationSettings/addSubject", {
      name: formVar.name,
      settingId: formVar.organizationSettingId,
    });
  } else {
    store.dispatch("OrganizationSettings/updateSubject", {
...
function submitDelete() {
  store.dispatch("OrganizationSettings/deleteSubject", {
    id: formVar.subjectId,
    status: "DELETED",
  });
}
