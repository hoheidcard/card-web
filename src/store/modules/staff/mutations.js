...
export const UPDATE_STAFF = (state, data) => {
  const objIndex = state.staffData.findIndex((obj) => obj.id === data.id);
  if (objIndex >= 0) {
    state.staffData[objIndex].name = data.name;
    state.staffData[objIndex].emailId = data.emailId;
    state.staffData[objIndex].dob = data.dob;
    state.staffData[objIndex].gender = data.gender;
    state.staffData[objIndex].designation = { name: data.designation };
  }
  state.staffModal = false;
};
export const UPDATE_STAFF_STATUS = (state, data) => {
  const objIndex = state.staffData.findIndex(
    (obj) => obj.accountId === data.id
  );
  if (objIndex >= 0) {
    state.staffData[objIndex].status = data.status;
  }
};
